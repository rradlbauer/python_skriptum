# 8.0 Fälle unterscheiden - bedingte Anweisungen
...
Eine `if`-Anweisung kann auch innerhalb einer anderen `if`- oder `else`-Anweisung stehen. Man spricht dann von verschachtelten if-Anweisungen. Damit lassen sich komplexere Entscheidungen treffen.

### Beispiel 1: Zahl prüfen (positiv, negativ, gerade/ungerade)

```python
zahl = int(input("Zahl eingeben: "))

if zahl >= 0:
    print("Die Zahl ist nicht negativ.")
    if zahl % 2 == 0:
...
### Beispiel 2: Benutzeranmeldung mit Passwortprüfung
...
### Beispiel 3: Note mit Zusatzbewertung
