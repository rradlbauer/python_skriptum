...
### Übersicht über häufige Funktionen und Module
In folgender Tabelle siehst du ein paar wichtige Funktionen und in welchen Modulen diese enthalten sind.

| Modul        | Funktion           | Beschreibung                                                      | Beispielaufruf                  |
|-------------|------------------|------------------------------------------------------------------|--------------------------------|
| math        | sqrt             | Berechnet die Quadratwurzel einer Zahl                            | `math.sqrt(16)`                |
| math        | ceil             | Rundet eine Zahl auf die nächsthöhere ganze Zahl auf              | `math.ceil(3.2)`               |
| math        | floor            | Rundet eine Zahl auf die nächstniedrigere ganze Zahl ab           | `math.floor(3.8)`              |
| math        | pi               | Gibt den Wert der Kreiszahl π zurück                              | `math.pi`                      |
| random      | randint          | Liefert eine zufällige ganze Zahl in einem Bereich                | `random.randint(1, 10)`        |
| random      | choice           | Wählt zufällig ein Element aus einer Liste                        | `random.choice([1,2,3])`       |
| random      | shuffle          | Mischt die Elemente einer Liste zufällig                            | `random.shuffle([1,2,3])`      |
| statistics  | mean             | Berechnet den Durchschnitt einer Liste von Zahlen                | `statistics.mean([1,2,3])`     |
| statistics  | median           | Berechnet den Median einer Liste von Zahlen                       | `statistics.median([1,2,3])`   |
| statistics  | stdev            | Berechnet die Standardabweichung einer Liste                      | `statistics.stdev([1,2,3])`    |
| datetime    | datetime.now     | Gibt das aktuelle Datum und die Uhrzeit zurück                     | `datetime.datetime.now()`       |
| datetime    | date.today       | Gibt das aktuelle Datum zurück (ohne Zeit)                        | `datetime.date.today()`         |
| os          | listdir          | Listet alle Dateien und Ordner in einem Verzeichnis auf           | `os.listdir('.')`               |
| os          | mkdir            | Erstellt ein neues Verzeichnis                                     | `os.mkdir('neuer_ordner')`     |
| os          | remove           | Löscht eine Datei                                                  | `os.remove('datei.txt')`       |
| sys         | exit             | Beendet das aktuelle Programm                                      | `sys.exit()`                    |
| sys         | argv             | Liefert eine Liste der Kommandozeilenargumente                     | `sys.argv`                      |
| re          | search           | Sucht nach einem Muster in einem Text                               | `re.search('abc', 'xyzabc')`   |
| re          | findall          | Findet alle Übereinstimmungen eines Musters im Text                 | `re.findall('a', 'banana')`    |
| re          | sub              | Ersetzt ein Muster durch einen anderen Text                         | `re.sub('a', 'x', 'banana')`   |
...
r vor dem String bedeutet, dass der String "roh" genommen wird, also ein Backslash "\\" nicht als Steuerzeichen gesehen wird.
...
## Übung
[Hier geht's zur Übung](../uebungen/UE_05.1_Module.md)
