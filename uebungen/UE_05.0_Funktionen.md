...
(Weil das Thema Schleifen erst später behandelt wird, kann angenommen werden,
dass die Zahl `n` maximal 5 Ziffern hat.)

#### Anleitung:

Man benötigt dafür zwei Ganzzahloperationen: 
 - _Modulo-Operation_ (Rest einer Ganzzahldivision) und
 - _Ganzzahldivision_.

Dabei geht man _von rechts nach links_ vor - d.h. man addiert die Ziffern von `n` schrittweise von rechts nach links:
Die jeweilige Ziffer mit dem aktuell kleinsten Stellenwert (Ziffer ganz rechts) bekommt man mit einer Modulo-Operation: 
`ziffer_ganz_rechts = n % 10`
Die restlichen Ziffern (weiter links) erhält man durch eine Ganzzahldivision:
`restliche_ziffern = n // 10`
Diese beiden Operationen sollen bis zu 5 mal hintereinander ausgeführt werden. 

__Wichtig__: Dabei muss man die Ergebnisse der Modulo-Operation __schrittweise addieren__.
