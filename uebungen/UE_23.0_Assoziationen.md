...
- blutdruck(self) -> tuple[float, float]:
...
![DICOM object model](../img/23.0/dicom.svg)
...
![Dicom Objekte](../img/23.0/dicom_objects.svg)
...
Für diese Übung gibt es [ein Juypter Notebook](../notebooks/Impfpass.ipynb) mit einer möglichen Musterlösung.
Musterlösung.

Erstelle die Klassen `Impfung` und `Impfpass` wie im folgenden
UML-Diagramm dargestellt:

![Impfpass](../img/23.0/impfpass.svg)

Ein `Impfpass` verwaltet in der Liste `impfungen` mehrere Impfungen.
Der Konstruktor der Klasse `Impfpass` bekommt als Parameter den 
Namen des Patienten übergeben.

Mit der Methode `neu(...)` wird eine Impfung zur Liste hinzugefügt.
Die Methode muss überprüfen, ob bereits eine `Impfung`
mit dem gleichen Impfstoff am selben Datum existiert.
In diesem Fall darf die `Impfung` nicht hinzugefügt werden.

Die Methode `impfstoffe()` gibt eine Liste aller verwendeten
Impfstoffe zurück (ohne Duplikate).

Die Methode `suche(...)` bekommt als Parameter den Namen eines Impfstoffs
und gibt eine Liste aller Impfungen mit diesem Impfstoff zurück.

Für jede `Impfung` werden folgende Daten gespeichert (Attribute):
- `datum`: Datum der Impfung (`date`)
- `impfstoff`: Name des Impfstoffs (`str`)
- `dosis`: Dosis der Impfung (`float`, in ml)
- `arzt`: Name des impfenden Arztes (`str`)

Der Konstruktor der Klasse `Impfung` bekommt alle diese Daten
außer dem Datum als Parameter übergeben. 
Das Datum wird automatisch auf das aktuelle Datum gesetzt.

Erstelle ein Python-Skript, 
in welchem ein Impfpass für den Patienten `Max Mustermann`
erstellt wird. Danach werden mit Hilfe der Methode `neu(...)`
folgende Impfungen hinzugefügt:

- Impfstoff: "Vaxigrip", Dosis: 0.3 ml, Arzt: "Dr. Meier", Datum: 2019-10-23
- Impfstoff: "Comirnaty", Dosis: 0.5 ml, Arzt: "Dr. Müller", Datum: 2021-05-01
- Impfstoff: "Vaxigrip", Dosis: 0.3 ml, Arzt: "Dr. Schmidt", Datum: 2020-11-05
- Impfstoff: "Vaxigrip", Dosis: 0.3 ml, Arzt: "Dr. Schmidt", Datum: 2021-10-15
- Impfstoff: "Comirnaty", Dosis: 0.5 ml, Arzt: "Dr. Müller", Datum: 2022-05-01
- Impfstoff: "Priorix", Dosis: 0.5 ml, Arzt: "Dr. Müller", Datum: 2019-03-01
- Impfstoff: "Comirnaty", Dosis: 0.5 ml, Arzt: "Dr. Müller", Datum: 2021-05-01 (doppelt, sollte nicht hinzugefügt werden)

Dann wird die alphabetisch sortierte Liste aller verwendeten 
Impfstoffe ausgegeben.

Danach wird in einer Schleife immer wieder nach einem Impfstoff 
gefragt, welchen der Benutzer eingeben soll.
Es wird dann jedes Mal die Liste aller Impfungen mit diesem Impfstoff
ausgegeben.
